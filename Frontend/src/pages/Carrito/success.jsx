...
const SuccessPage = () => {
  ...
  useEffect(() => {
    ...
    if (finalPaymentId) {
      ...
      axios.get(`${apiUrl}/payments/transaction/${finalPaymentId}`)
        .then(response => {
        ...
        .catch(err => {
          ...
          const transactionFromUrl = {
            ...
            // Intentar obtener más datos de los parámetros
        ...
        <div className="md:flex">
          ...
          <div className="md:w-3/5 p-8 md:p-12 text-center">
            ...
            {transaction && (
              <div className="mb-6 text-left bg-gray-50 p-4 rounded-lg">
                ...
                <div className="space-y-2">
                  <p><strong>ID de pago:</strong> {transaction.payment_id || 'No disponible'}</p>
                  
                  <p><strong>Referencia:</strong> {transaction.external_reference || 'No disponible'}</p>
                  
                  <p><strong>Monto:</strong> {formatAmount(transaction.amount)}</p>
                  ...
                  <p><strong>Tipo de pago:</strong> {transaction.payment_type || 'No disponible'}</p>
                  ...
                  {transaction.merchant_order_id && (
                    <p><strong>Orden:</strong> {transaction.merchant_order_id}</p>
                  )}
                  
                  {transaction.collection_status && (
                    <p><strong>Estado de colección:</strong> {transaction.collection_status}</p>
                  )}
                </div>
              </div>
            )}
            ...
            <p className="text-lg text-gray-600 mb-6">
              Tu pago ha sido procesado correctamente. Pronto recibirás una confirmación por email.
            </p>
